// verify-pepe-smart.js
// Self-verifying stxer mainnet-fork harness for pepe-smart-faktory (DRAFT).
// Generated from verify-mia-smart.js; same coverage minus the DLMM path
// (these routers, like b-smart, have none).
//
...
const C = `${DEPLOYER}.pepe-smart-faktory`;
...
const MIA_HOLDER = "SP2022PJ05WB4VXP8HTVFAFE186AM94A4WYQ1RQY2"; // 100k PEPE (3dec); holder has 710k

const SBTC_AMOUNT = 100000n; // 100k sats
const STX_AMOUNT = 100000000n; // 100 STX (6dec)
const MIA_AMOUNT = 100000000n; // 100k PEPE (3dec); holder has 710k
...
const b = SimulationBuilder.new()
  .withSender(DEPLOYER)
  .addContractDeploy({
    contract_name: "pepe-smart-faktory",
    source_code: src,
    // Clarity 5: as-contract? with with-ft / with-stx allowances.
    clarity_version: ClarityVersion.Clarity5,
  });
steps.push({ label: "deploy pepe-smart-faktory (Clarity 5)", kind: "deploy" });
...
for (const [label, code] of [
  ...
  ["sim stx->token (bitflow)", `(simulate-stx-to-token u${STX_AMOUNT} true)`],
  ["sim stx->token (velar)", `(simulate-stx-to-token u${STX_AMOUNT} false)`],
  ["sim token->stx (bitflow)", `(simulate-token-to-stx u${MIA_AMOUNT} true)`],
  ["sim token->stx (velar)", `(simulate-token-to-stx u${MIA_AMOUNT} false)`],

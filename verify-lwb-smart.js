// verify-lwb-smart.js
// Self-verifying stxer mainnet-fork harness for lwb-smart-faktory (DRAFT).
// Generated from verify-flat-smart.js; venues: lwb-faktory-pool via fakfun-core-v2
// (the "fak" leg) and bitflow xyk STX/LWB (x = STX) (the "alex" leg).
//
...
const C = `${DEPLOYER}.lwb-smart-faktory`;
...
const MIA_HOLDER = "SPZ2X7Q0Z69KTN7SZF90MR9AZGZ2ETXFGMFXKKN8"; // ~1.79B LWB (6dec)

const SBTC_AMOUNT = 100000n; // 100k sats
const STX_AMOUNT = 100000000n; // 100 STX (6dec)
const MIA_AMOUNT = 10000000000000n; // 10M LWB (6dec)
...
// SRC=./contracts/d-lwb-smart-faktory.clar node ... runs the comment-stripped deploy variant.
...
const b = SimulationBuilder.new({ stacksNodeAPI: process.env.STACKS_API || "http://77.42.3.101/stacks-api" })
  .withSender(DEPLOYER)
  .addContractDeploy({
    contract_name: "lwb-smart-faktory",
    source_code: src,
    // Clarity 5: as-contract? with with-ft / with-stx allowances.
    clarity_version: ClarityVersion.Clarity5,
  });
steps.push({ label: "deploy lwb-smart-faktory (Clarity 5)", kind: "deploy" });
...
for (const [label, code] of [
  ...
  ["sim token->sbtc (fak pool)", `(simulate-token-to-sbtc u${MIA_AMOUNT})`],
...
for (const [label, code] of [
  ...
  ["residue token", "(contract-call? 'SP277HZA8AGXV42MZKDW5B2NNN61RHQ42MTAHVNB1.little-whiny-bitch-stxcity get-balance current-contract)"],
]) {
  b.addEvalCode(C, code);

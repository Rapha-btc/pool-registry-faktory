;; SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.rock-smart-faktory (DRAFT - not deployed)
;;
;; Smart split router for ROCK (stacks-rock, asset "rock", 6 dec), modeled on the
...
;; Venue map:
;;   - "fak" leg:  rock-faktory-pool-2 via fakfun-core-v2 execute (gated pool,
;;     core-v2 approved; LP_REBATE 0.3% inside the quote, sell fee field is 0 so
;;     dy is paid in full - no haircut). The empty rock-faktory-pool (v1) is NOT used.
;;   - STX-side DEX leg: Velar old-core pool 18 wstx/rock via univ2-router
;;     (reserve0 = wstx, reserve1 = rock). There is no BitFlow ROCK pool, so
;;     `flag` only picks the sBTC<->STX bridge: true = BitFlow xyk sbtc-stx,
;;     false = Velar univ2-pool-0070 (as flatearth-smart).
;; The dex split ratio weighs the fak pool's sBTC side against the Velar pool's
;; STX reserve. Safety is always the signed min-out, never the ratio.
...
(define-constant POOL 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.rock-faktory-pool-2)
...
;; Velar wstx/rock pool id (reserve0 = wstx, reserve1 = rock).
...
;; STX reserve of the Velar wstx/rock pool, native uSTX. flag is unused here
...
;; rock-faktory-pool-2 is NOT registered in fakfun-core-v2 (core execute returns
;; ERR_POOL_NOT_FOUND u1003), so the fak leg calls the pool's own execute. The
;; pool is gated but its is-approved-caller passes when tx-sender equals
;; contract-caller, which is the case for this router inside as-contract?.
...
;; STX -> ROCK: univ2-router over Velar pool 18 (wstx -> rock). flag unused.
...
;; ROCK -> STX: Velar pool 18 rock -> wstx. flag unused.
...
(define-read-only (simulate-sbtc-to-token (sbtc-amount uint))
  (get dy
    (unwrap-panic (contract-call?
      'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.rock-faktory-pool-2 quote
...
;; STX -> ROCK quote: Velar pool 18 reserves (r0 = wstx, r1 = rock) with the
;; pool's own swap-fee num/den. flag unused.
...
;; ROCK -> STX quote, the other way round.

...
(define-public (register-pool
  ...
  (let (
      ...
      (caller tx-sender)
      (reserves-data (unwrap! (contract-call? pool-contract quote u0 (some OP_LOOKUP_RESERVES))
        ERROR_RESERVES
      ))
    )
    ...
    (asserts! (is-none (map-get? pool-contracts (contract-of pool-contract)))
  ...
  (let (
    ...
    (if (and (is-some pool-id) (is-some pool-info))
      (let (
          ...
          (reserves-data (unwrap!
            (contract-call? pool-contract quote u0 (some OP_LOOKUP_RESERVES))
            ERROR_RESERVES
          ))
        )
        (ok (some {
...
(define-public (execute
  ...
  (let (
      ...
      (reserves-after (unwrap! (contract-call? pool-contract quote u0 (some OP_LOOKUP_RESERVES))
        ERROR_RESERVES
      ))
    )
          ...
          (if (is-eq operation OP_SWAP_B_TO_A)
            ...
            (if (is-eq operation OP_ADD_LIQUIDITY)
              ...
              (if (is-eq operation OP_REMOVE_LIQUIDITY)
                ...
                (asserts! false ERR_INVALID_OPERATION) ;; Unknown operation
              )
            )
          )
        )

...
(define-constant TOKEN 'SP277HZA8AGXV42MZKDW5B2NNN61RHQ42MTAHVNB1.little-whiny-bitch-stxcity)
(define-constant SBTC 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token)

(define-constant TOKEN-ASSET "LWB")
(define-constant SBTC-ASSET "sbtc-token")

(define-constant CORE 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.fakfun-core-v2)
(define-constant POOL 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.lwb-faktory-pool)
...
(define-constant XYK-TOKEN-POOL 'SP277HZA8AGXV42MZKDW5B2NNN61RHQ42MTAHVNB1.xyk-pool-stx-lwb-v-1-1)
...
(define-read-only (get-fak-sbtc-token-liquidity)
  (let ((pool-data (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.lwb-faktory-pool
      get-reserves-quote
    )))
    (get dx pool-data)
  )
...
(define-read-only (get-dex-stx-token-liquidity (flag bool))
  (get x-balance
    (unwrap-panic (contract-call?
      'SP277HZA8AGXV42MZKDW5B2NNN61RHQ42MTAHVNB1.xyk-pool-stx-lwb-v-1-1
      get-pool
    ))
  )
)
...
(define-private (swap-stx-to-token
  ...
  (let ((dy (try! (contract-call? XYK-CORE swap-x-for-y XYK-TOKEN-POOL STX-TOKEN TOKEN
      stx-amount u1
    ))))
    (ok dy)
  )
...
(define-private (swap-token-to-stx
  ...
  (let ((dx (try! (contract-call? XYK-CORE swap-y-for-x XYK-TOKEN-POOL STX-TOKEN TOKEN
      token-amount u1
    ))))
    (ok dx)
  )
...
(define-read-only (simulate-token-to-sbtc (token-amount uint))
  (let ((raw-dy (get dy
      (unwrap-panic (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.lwb-faktory-pool
...
(define-read-only (simulate-token-to-stx
  ...
  (let (
      ...
      (provider-fee (get y-provider-fee pool))
      (BPS u10000)
      (y-amount-fees-protocol (/ (* token-amount protocol-fee) BPS))
      (y-amount-fees-provider (/ (* token-amount provider-fee) BPS))
      (y-amount-fees-total (+ y-amount-fees-protocol y-amount-fees-provider))
      (dy (- token-amount y-amount-fees-total))
      (updated-y-balance (+ y-balance dy))
      (dx (/ (* x-balance dy) updated-y-balance))
    )
    dx

;; SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.fakfun-smart-faktory (DRAFT - not deployed)
;;
;; Smart split router for FAKFUN, derived from the deployed FlatEarth router
;; with the same public interface (drop-in for the FE panel + backend
;; SMART_ROUTERS). Venue map:
;;   - "fak" leg:  Charisma-hosted sBTC pool (POOL), via fakfun-core-v2
;;     execute (the pool is registered there), so smart trades print via the
;;     core and the indexer sees them. The Charisma pool transfers its net dy
;;     exactly, so the template's post-execute fee haircut does not apply.
;;   - "alex" leg: bitflow XYK-TOKEN-POOL (FAKFUN/STX, x = FAKFUN, y = STX)
;;     via XYK-CORE, replacing the template's Velar token pool. The "alex"
;;     naming is kept only for interface parity. STX amounts are plain ustx.
...
(define-constant POOL 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.sbtc-fakfun-amm-lp-v1)
...
(define-constant XYK-TOKEN-POOL 'SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.xyk-pool-fakfun-stx-v-1-1)
...
(define-private (swap-token-to-sbtc (token-amount uint))
  (let ((result (try! (contract-call? CORE execute POOL token-amount (some 0x01)))))
    (ok (get dy result))
  )
)
...
;; STX -> FAKFUN on the bitflow xyk (x = FAKFUN, y = STX): swap-y-for-x.
;; flag kept for interface parity with the template.
...
;; FAKFUN -> STX on the bitflow xyk: swap-x-for-y.
...
;; STX -> FAKFUN on bitflow fakfun-stx (y = STX in, x = FAKFUN out).
...
;; FAKFUN -> STX on bitflow fakfun-stx (x = FAKFUN in, y = STX out).

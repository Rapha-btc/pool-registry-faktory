...
(define-private (swap-stx-to-token
  ...
  (let ((result (try! (contract-call? VELAR-TOKEN-POOL swap WSTX TOKEN VELAR-TOKEN-FEES stx-amount
      u1
    ))))
...
(define-private (swap-token-to-stx
  ...
  (let ((result (try! (contract-call? VELAR-TOKEN-POOL swap TOKEN WSTX VELAR-TOKEN-FEES
      token-amount u1
    ))))
    (ok (get amt-out result))
  )

;; SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.flatearth-smart-faktory
;; DRAFT - modeled on mia-smart-faktory (deployed, Clarity 5). Differences:
;;   - token: FlatEarth (flat-earth-stxcity, asset "FlatEarth", 6 dec)
;;   - fak leg: fakfun-core-v2.execute on flatearth-faktory-pool-v2 (same pool
;;     the deployed flatearth-arbitrage-faktory-v3 routes through)
;;   - STX-side DEX leg is Velar univ2-pool-0003 ONLY (reserve0 = STX, reserve1 =
;;     FLAT - the orientation that bit flatearth-arb-v2). BitFlow's
;;     xyk-pool-stx-flat is dust (~22 STX) and is not wired.
;;     So `flag` only picks the sBTC<->STX bridge: true = BitFlow xyk, false = Velar 0070.
;;   - native units end to end, none of mia-smart's ALEX 8-dec scaling
;;   - fak sell leg shaves the 0.1% faktory fee off dy exactly as flatearth-arb-v3 does
...
(define-constant TOKEN 'SP3W69VDG9VTZNG7NTW1QNCC1W45SNY98W1JSZBJH.flat-earth-stxcity)
...
;; name: flat-earth-stxcity declares "FlatEarth". Getting one wrong makes every
;; transfer under that allowance abort.
(define-constant TOKEN-ASSET "FlatEarth")
(define-constant SBTC-ASSET "sbtc-token")

(define-constant CORE 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.fakfun-core-v2)
(define-constant POOL 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.flatearth-faktory-pool-v2)
...
(define-constant VELAR-TOKEN-POOL 'SP1Y5YSTAHZ88XYK1VPDH24GY0HPX5J4JECTMY4A1.univ2-pool-v1_0_0-0003)
(define-constant VELAR-TOKEN-FEES 'SP1Y5YSTAHZ88XYK1VPDH24GY0HPX5J4JECTMY4A1.univ2-fees-v1_0_0-0003)
...
;; STX sitting on Velar pool 0003 (reserve0), native uSTX. `flag` unused: one venue.
...
;; STX -> FLAT on Velar pool 0003. `flag` is accepted for interface parity with
;; the other routers; there is one FLAT/STX venue worth routing to.
...
;; Velar 0003 quotes. reserve0 = STX, reserve1 = FLAT (verified; the v2 arb
;; had these backwards and its checks returned garbage). Fee 0.3%.
...
(define-read-only (simulate-token-to-stx (token-amount uint) (flag bool))
  (let ((pool (unwrap-panic (contract-call?
        ...
        (r0 (get reserve0 pool))
        (r1 (get reserve1 pool))
        (amt-in-adjusted (/ (* token-amount u997) u1000))

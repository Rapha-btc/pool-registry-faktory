;; rock-arb-faktory (DRAFT - not deployed, not committed)
;; Arbitrage: ROCK -> sBTC -> STX -> ROCK and reverse, derived from leo-arbitrage-faktory-v2.
;; ROCK has one TOKEN-STX DEX: Velar old-core pool 18 wstx/rock via univ2-router, so the
;; four routes differ only by the sBTC<->STX bridge (bit = Bitflow xyk sbtc-stx, vel = Velar
;; univ2-pool-0070). The fak leg calls rock-faktory-pool-2.execute DIRECTLY: the pool is not
;; registered in fakfun-core-v2 (core execute returns ERR_POOL_NOT_FOUND u1003) and its own
;; gate passes when tx-sender equals contract-caller, which as-contract gives us.
;; Sells on the fak pool pay dy - 0.1% faktory fee, so dy is shaved by 0.1% (as pepe-arb-v3).
;; Keeps profits (paid to DEPLOYER). Clarity 5: every leg runs under as-contract? with an explicit allowance,
;; payouts use current-contract (no CONTRACT constant).
...
;; ROCK -> sBTC (fak) -> STX (bitflow) -> ROCK (velar)
(define-public (arb-fak-bit-vel
  ...
  (begin
    ...
    (let ((sbtc-out (try! (as-contract? ((with-ft 'SP4M2C88EE8RQZPYTC4PZ88CE16YGP825EYF6KBQ.stacks-rock "rock" amt-in)) (try! (swap-token-to-sbtc amt-in)))))
          ...
          (try! (as-contract? ((with-ft 'SP4M2C88EE8RQZPYTC4PZ88CE16YGP825EYF6KBQ.stacks-rock "rock" amt-out))
            (try! (contract-call? 'SP4M2C88EE8RQZPYTC4PZ88CE16YGP825EYF6KBQ.stacks-rock transfer amt-out current-contract DEPLOYER none))
          ))
          (ok {
            token-in: amt-in,
            token-out: amt-out
...
;; ROCK -> sBTC (fak) -> STX (velar) -> ROCK (velar)
...
;; ROCK -> STX (velar) -> sBTC (bitflow) -> ROCK (fak)
(define-public (arb-vel-bit-fak
  ...
  (begin
    ...
    (let ((stx-out (try! (as-contract? ((with-ft 'SP4M2C88EE8RQZPYTC4PZ88CE16YGP825EYF6KBQ.stacks-rock "rock" amt-in)) (try! (swap-token-to-stx-velar amt-in)))))
...
;; ROCK -> STX (velar) -> sBTC (velar) -> ROCK (fak)
...
;; --- fak leg: rock-faktory-pool-2 called directly ---

(define-private (swap-token-to-sbtc (amount uint))
  (let (
      (result (try! (contract-call?
        'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.rock-faktory-pool-2
        execute
        amount
        (some 0x01)
      )))
...
;; --- STX <-> ROCK on Velar old-core pool 18 ---
...
;; get-swap-quote (bare tuple) carries the output-side fee; net = dy - fee
...
;; A-to-B: quote already nets FAKTORY_FEE from the sBTC input
...
(define-public (rescue-token (amount uint))
  (begin
    (asserts! (is-eq tx-sender DEPLOYER) ERR-NOT-AUTHORIZED)
    (as-contract? ((with-ft 'SP4M2C88EE8RQZPYTC4PZ88CE16YGP825EYF6KBQ.stacks-rock "rock" amount))
      (try! (contract-call? 'SP4M2C88EE8RQZPYTC4PZ88CE16YGP825EYF6KBQ.stacks-rock transfer amount current-contract DEPLOYER none))
    )
  )
)

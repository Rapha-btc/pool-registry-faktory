      ...
      (let (
          ...
          (token-from-alex (if (> stx-from-dex u0)
            (try! (as-contract? ((with-stx stx-from-dex))
              (try! (swap-stx-to-token (* stx-from-dex u100)))
            ))
            u0
          ))
      ...
      (let (
          ...
          (token-from-alex (if (> alex-amount u0)
            (try! (as-contract? ((with-stx alex-amount))
              (try! (swap-stx-to-token (* alex-amount u100)))
            ))
            u0
          ))
    ...
    (let (
      ...
      (let (
          ...
          (sbtc-from-fak (if (> fak-amount u0)
            (try! (as-contract? ((with-ft MIA MIA-ASSET fak-amount))
              (try! (swap-token-to-sbtc fak-amount))
            ))
            u0
          ))
          (stx-from-alex (if (> alex-amount u0)
            (try! (as-contract? ((with-ft MIA MIA-ASSET alex-amount))
              (try! (swap-token-to-stx alex-amount))
            ))
            u0
          ))
          (sbtc-from-dex (if (> stx-from-alex u0)
            (if flag
              (try! (as-contract? ((with-stx (/ stx-from-alex u100)))
                (try! (swap-stx-to-sbtc (/ stx-from-alex u100)))
              ))
              (try! (as-contract? ((with-stx (/ stx-from-alex u100)))
                (try! (swap-stx-to-sbtc-velar (/ stx-from-alex u100)))
              ))
            )
            u0
          ))
...
(define-read-only (calculate-optimal-ratio-sbtc-to-token (flag bool))
  (let (
      ...
      (alex-stx-token-in-sbtc (if (> y-balance u0)
        (/
          (* (/ alex-stx-token-liquidity u100) (get x-balance sbtc-stx-liquidity))
          y-balance
        )
        u0
      ))
...
(define-read-only (calculate-optimal-ratio-stx-to-token (flag bool))
  (let (
      ...
      (alex-percentage (if (> total-liquidity u0)
        (/ alex-stx-token-liquidity total-liquidity)
        u0
      ))
    )
    {
...
(define-read-only (get-fak-sbtc-token-liquidity)
  (let ((pool-data (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.mia-pool-faktory
      get-reserves-quote
    )))
    (get dx pool-data)
  )
...
(define-read-only (get-alex-stx-token-liquidity)
  (let ((pool-data (unwrap-panic (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01
      get-pool-details
...
(define-private (swap-stx-to-token (stx-amount uint))
  (let ((result (try! (contract-call? ALEX-POOL swap-x-for-y WSTX-V2 WMIA u100000000 stx-amount
      none
    ))))
...
(define-private (swap-token-to-stx (token-amount uint))
  (let ((result (try! (contract-call? ALEX-POOL swap-y-for-x WSTX-V2 WMIA u100000000
      (* token-amount u100) none
    ))))
    (ok (get dx result))
  )
)
...
(define-private (swap-sbtc-to-stx-dlmm
  ...
  (let ((res (try! (if (is-eq pool-ver u1)
      (contract-call? DLMM-ROUTER swap-y-for-x-simple-multi DLMM-POOL-1 STX-TOKEN
        SBTC sbtc-amount u1 none
      )
      (if (is-eq pool-ver u2)
        (contract-call? DLMM-ROUTER swap-y-for-x-simple-multi DLMM-POOL-2
          STX-TOKEN SBTC sbtc-amount u1 none
        )
        (contract-call? DLMM-ROUTER swap-y-for-x-simple-multi DLMM-POOL-3
          STX-TOKEN SBTC sbtc-amount u1 none
        )
      )
    ))))
...
(define-private (swap-stx-to-sbtc-dlmm
  ...
  (let ((res (try! (if (is-eq pool-ver u1)
      (contract-call? DLMM-ROUTER swap-x-for-y-simple-multi DLMM-POOL-1 STX-TOKEN
        SBTC stx-amount u1 none
      )
      (if (is-eq pool-ver u2)
        (contract-call? DLMM-ROUTER swap-x-for-y-simple-multi DLMM-POOL-2
          STX-TOKEN SBTC stx-amount u1 none
        )
        (contract-call? DLMM-ROUTER swap-x-for-y-simple-multi DLMM-POOL-3
          STX-TOKEN SBTC stx-amount u1 none
        )
      )
    ))))
  ...
  (begin
    ...
    (let (
      ...
      (let (
          ...
          (stx-from-dex (if (> alex-amount u0)
            (try! (as-contract? ((with-ft SBTC SBTC-ASSET alex-amount))
              (try! (swap-sbtc-to-stx-dlmm dlmm-pool alex-amount))
            ))
            u0
          ))
          (token-from-alex (if (> stx-from-dex u0)
  ...
  (begin
    ...
    (let (
      ...
      (let (
          ...
          (sbtc-from-dex (if (> fak-amount u0)
            (try! (as-contract? ((with-stx fak-amount))
              (try! (swap-stx-to-sbtc-dlmm dlmm-pool fak-amount))
            ))
            u0
          ))
          (token-from-fak (if (> sbtc-from-dex u0)
  ...
  (begin
    ...
    (let (
      ...
      (let (
          ...
          (sbtc-from-dex (if (> stx-from-alex u0)
            (try! (as-contract? ((with-stx (/ stx-from-alex u100)))
              (try! (swap-stx-to-sbtc-dlmm dlmm-pool (/ stx-from-alex u100)))
            ))
            u0
          ))
          (total-sbtc-out (+ sbtc-from-fak sbtc-from-dex))
  ...
  (begin
    ...
    (let (
      ...
      (let (
          ...
          (stx-from-dex (if (> sbtc-from-fak u0)
            (try! (as-contract? ((with-ft SBTC SBTC-ASSET sbtc-from-fak))
              (try! (swap-sbtc-to-stx-dlmm dlmm-pool sbtc-from-fak))
            ))
            u0
          ))
          (total-stx-out (+ (/ stx-from-alex u100) stx-from-dex))

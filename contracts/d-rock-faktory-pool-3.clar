...
(define-read-only (quote
  ...
  (let ((operation (get-byte (default-to 0x00 opcode) u0)))
    (if (is-eq operation OP_SWAP_A_TO_B)
      (let ((sq (get-swap-quote amount (some 0x00))))
        (ok {
          dx: (get dx sq),
          dy: (get dy sq),
          dk: u0,
      ...
      (if (is-eq operation OP_SWAP_B_TO_A)
        (let ((sq (get-swap-quote amount (some 0x01))))
          (ok {
            dx: (get dx sq),
            dy: (get dy sq),
            dk: u0,
...
(define-public (swap-a-to-b
  ...
  (let (
    ...
    (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
      transfer (- amount fee-d) sender current-contract none
    ))
    (if (> fee-d u0)
      (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
        transfer fee-d sender FAKTORY_ADDRESS none
      ))
      true
    )
    (try! (as-contract?
      ...
      (try! (contract-call? 'SP4M2C88EE8RQZPYTC4PZ88CE16YGP825EYF6KBQ.stacks-rock
        transfer dy-d current-contract sender none
      ))
    ))
    (print {
      type: "buy",
...
(define-public (swap-b-to-a
  ...
  (let (
    ...
    (try! (as-contract?
      ...
      (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
        transfer (- dy-d fee-d) current-contract sender none
      ))
      (if (> fee-d u0)
        (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
          transfer fee-d current-contract FAKTORY_ADDRESS none
        ))
        true
      )))
    (print {
...
(define-public (add-liquidity (amount uint))
  (let (
    ...
    (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
      transfer dx-d sender current-contract none
    ))
    (try! (contract-call? 'SP4M2C88EE8RQZPYTC4PZ88CE16YGP825EYF6KBQ.stacks-rock
      transfer dy-d sender current-contract none
    ))
...
(define-public (remove-liquidity (amount uint))
  (let (
    ...
    (try! (as-contract?
      ...
      (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
        transfer dx-d current-contract sender none
      ))
      (try! (contract-call? 'SP4M2C88EE8RQZPYTC4PZ88CE16YGP825EYF6KBQ.stacks-rock
        transfer dy-d current-contract sender none
      ))
...
(define-private (get-reserves)
  {
    ...
    b: (unwrap-panic (contract-call? 'SP4M2C88EE8RQZPYTC4PZ88CE16YGP825EYF6KBQ.stacks-rock
      get-balance current-contract
    )),
  }
)
...
(define-public (initialize-pool
  ...
  (begin
    ...
    (try! (contract-call? 'SP4M2C88EE8RQZPYTC4PZ88CE16YGP825EYF6KBQ.stacks-rock
      transfer highest contract-caller current-contract none
    ))
    (print {

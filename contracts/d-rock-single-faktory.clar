...
(define-public (deposit-sbtc-for-lp (lp-amount uint))
  (let (
      ...
      (lp-result (try! (as-contract?
        (
          (with-ft SBTC "sbtc-token" sbtc-needed)
    ...
    (asserts! (< burn-block-height (+ (var-get creation-block) ENTRY_PERIOD))
      ERR_TOO_LATE
    )
...
(define-private (do-withdraw (user principal))
  (let (
    ...
    (let (
        (remove-result (try! (as-contract? ((with-ft POOL "sBTC-rock" user-lp))
      ...
      (try! (as-contract? ((with-ft SBTC "sbtc-token" sbtc-received))
        (begin
          (try! (contract-call? SBTC transfer user-sbtc-share current-contract user
            none
          ))
          (try! (contract-call? SBTC transfer depositor-sbtc-share current-contract
            depositor-principal none
          ))
        )))
      (try! (as-contract? ((with-ft ROCK "rock" token-received))
        (begin
          (try! (contract-call? ROCK transfer user-token-share current-contract user
            none
          ))
          (try! (contract-call? ROCK transfer depositor-token-share current-contract
            depositor-principal none
          ))
        )))
...
(define-public (withdraw-remaining-token)
  (let (
      (entry-end-block (+ (var-get creation-block) ENTRY_PERIOD))
      (depositor-principal (unwrap! (var-get depositor) ERR_NOT_INITIALIZED))
    )
    ...
    (let ((remaining-token (- (var-get initial-token-amount) (var-get token-used-for-lp))))
      (and
        ...
        (try! (as-contract? ((with-ft ROCK "rock" remaining-token))
          (try! (contract-call? ROCK transfer remaining-token current-contract
            depositor-principal none
          ))
        ))
      )

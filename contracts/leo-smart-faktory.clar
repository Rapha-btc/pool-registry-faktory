;; SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.leo-smart-faktory (DRAFT - not deployed)
;;
;; Smart split router for LEO, derived from the deployed FlatEarth router with
;; the same public interface (drop-in for the FE panel + backend
;; SMART_ROUTERS). Venue map:
;;   - "fak" leg:  leo-faktory-pool-v2 via fakfun-core-v2 execute (same
;;     template family as the FlatEarth pool, so the 0.1% post-execute
;;     haircut on sells is kept).
;;   - "alex" leg: ALEX 2-hop STX <-> ALEX <-> LEO through amm-pool-v2-01
;;     swap-helper-a (the deepest LEO/STX venue, ~$65k), lifted from the
;;     deployed leo-arbitrage-faktory-v2. ALEX is 8-dec: ustx and LEO (6-dec)
;;     are scaled *100 in and /100 out. The other LEO/STX venues (bitflow xyk
;;     leo-stx, bitflow DLMM leo-stx bps-50, Velar pool 28) are not routed
;;     here; the arb contract covers them.
;;   - sBTC<->STX bridge: bitflow sbtc-stx xyk (flag=true) or Velar univ2
;;     (flag=false), unchanged.
;; The dex split ratio uses the STX depth of the ALEX STX/ALEX hop as the
;; venue's liquidity proxy; safety is always the signed min-out, not the ratio.
...
(define-constant TOKEN 'SP1AY6K3PQV5MRT6R4S671NWW2FRVPKM0BR162CT6.leo-token)
(define-constant SBTC 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token)

(define-constant TOKEN-ASSET "leo")
(define-constant SBTC-ASSET "sbtc-token")

(define-constant CORE 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.fakfun-core-v2)
(define-constant POOL 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.leo-faktory-pool-v2)
...
(define-constant ALEX-WLEO 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-wleo)
(define-constant ALEX-FACTOR u100000000)
;; ALEX's 2-hop helper routes the intermediate ALEX plus the wstx / wleo
...
;; so the cap only bounds what ALEX's own code can move within that call.
    ...
    (let (
      ...
      (let (
          ...
          (token-from-alex (if (> stx-from-dex u0)
            (try! (as-contract?
              ((with-stx stx-from-dex) (with-ft ALEX-TOKEN "alex" ALEX-PASSTHROUGH) (with-ft ALEX-WSTX "wstx" ALEX-PASSTHROUGH) (with-ft ALEX-WLEO "wleo" ALEX-PASSTHROUGH))
    ...
    (let (
      ...
      (let (
          ...
          (token-from-alex (if (> alex-amount u0)
            (try! (as-contract?
              ((with-stx alex-amount) (with-ft ALEX-TOKEN "alex" ALEX-PASSTHROUGH) (with-ft ALEX-WSTX "wstx" ALEX-PASSTHROUGH) (with-ft ALEX-WLEO "wleo" ALEX-PASSTHROUGH))
    ...
    (let (
      ...
      (let (
          ...
          (stx-from-alex (if (> alex-amount u0)
            (try! (as-contract?
              ((with-ft TOKEN TOKEN-ASSET alex-amount) (with-ft ALEX-TOKEN "alex" ALEX-PASSTHROUGH) (with-ft ALEX-WSTX "wstx" ALEX-PASSTHROUGH) (with-ft ALEX-WLEO "wleo" ALEX-PASSTHROUGH))
...
(define-read-only (get-fak-sbtc-token-liquidity)
  (let ((pool-data (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.leo-faktory-pool-v2
      get-reserves-quote
    )))
    (get dx pool-data)
  )
...
;; STX -> ALEX -> LEO on ALEX (8-dec: *100 in, /100 out). flag kept for
;; interface parity with the template.
(define-private (swap-stx-to-token
    (stx-amount uint)
    (flag bool)
  )
  (let ((token-8dec (try! (contract-call? ALEX-AMM swap-helper-a ALEX-WSTX ALEX-TOKEN ALEX-WLEO
      ALEX-FACTOR ALEX-FACTOR (* stx-amount u100) none
    ))))
    (ok (/ token-8dec u100))
  )
)

;; LEO -> ALEX -> STX on ALEX (8-dec: *100 in, /100 out).
(define-private (swap-token-to-stx
    (token-amount uint)
    (flag bool)
  )
  (let ((stx-8dec (try! (contract-call? ALEX-AMM swap-helper-a ALEX-WLEO ALEX-TOKEN ALEX-WSTX
      ALEX-FACTOR ALEX-FACTOR (* token-amount u100) none
    ))))
    (ok (/ stx-8dec u100))
  )
)
...
(define-read-only (simulate-token-to-sbtc (token-amount uint))
  (let ((raw-dy (get dy
      (unwrap-panic (contract-call?
        'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.leo-faktory-pool-v2 quote
...
(define-read-only (simulate-sbtc-to-token (sbtc-amount uint))
  (get dy
    (unwrap-panic (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.leo-faktory-pool-v2
...
(define-read-only (simulate-token-to-stx
  ...
  (/
    (unwrap-panic (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01
      get-helper-a 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-wleo
      'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-alex

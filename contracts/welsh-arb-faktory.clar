;; welsh-arb-faktory (DRAFT - not deployed, not committed)
;; Arbitrage: WELSH -> sBTC -> STX -> WELSH and reverse, derived from leo-arbitrage-faktory-v2.
;; Routes through fakfun-core-v2 on welshcorgicoin-faktory-pool-v2 (emits DB events).
;; WELSH has three TOKEN-STX DEXes: Bitflow xyk-pool-welsh-stx-v-1-1 (x = WELSH, y = STX;
;; routes 1-2), Velar old-core pool 27 wstx/welsh (dust, ~2.7k STX; routes 3-4), ALEX 2-hop
;; STX <-> ALEX <-> WELSH via token-wcorgi (routes 5-8, 8-dec scaled).
;; Keeps profits (paid to DEPLOYER). Clarity 3 like the WELSH arb (as-contract in define-constant).
...
(define-constant VELAR-POOL-ID u27)
...
(define-public (arb-fak-bit-bit
  ...
  (begin
    ...
    (let ((sbtc-out (try! (as-contract? ((with-ft 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token "welshcorgicoin" amt-in)) (try! (swap-token-to-sbtc amt-in)))))
          (stx-out (try! (as-contract? ((with-ft 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token "sbtc-token" sbtc-out)) (try! (swap-sbtc-to-stx sbtc-out)))))
          (amt-out (try! (as-contract? ((with-stx stx-out)) (try! (swap-stx-to-token-bitflow stx-out))))))
          (asserts! (>= amt-out min-amt-out) ERR-SLIPPAGE)
          (asserts! (> amt-out amt-in) ERR-NO-PROFIT)
          (try! (as-contract? ((with-ft 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token "welshcorgicoin" amt-out))
            (try! (contract-call? 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token transfer amt-out current-contract DEPLOYER none))
          ))
          (ok {
            token-in: amt-in,
            token-out: amt-out
...
(define-public (arb-fak-vel-vel
  ...
  (begin
    ...
    (let ((sbtc-out (try! (as-contract? ((with-ft 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token "welshcorgicoin" amt-in)) (try! (swap-token-to-sbtc amt-in)))))
          (stx-out (try! (as-contract? ((with-ft 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token "sbtc-token" sbtc-out)) (try! (swap-sbtc-to-stx-velar sbtc-out)))))
          (amt-out (try! (as-contract? ((with-stx stx-out)) (try! (swap-stx-to-token-velar stx-out))))))
          (asserts! (>= amt-out min-amt-out) ERR-SLIPPAGE)
          (asserts! (> amt-out amt-in) ERR-NO-PROFIT)
          (try! (as-contract? ((with-ft 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token "welshcorgicoin" amt-out))
            (try! (contract-call? 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token transfer amt-out current-contract DEPLOYER none))
...
(define-public (arb-bit-bit-fak
  ...
  (begin
    ...
    (let ((stx-out (try! (as-contract? ((with-ft 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token "welshcorgicoin" amt-in)) (try! (swap-token-to-stx-bitflow amt-in)))))
          (sbtc-out (try! (as-contract? ((with-stx stx-out)) (try! (swap-stx-to-sbtc stx-out)))))
          (amt-out (try! (as-contract? ((with-ft 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token "sbtc-token" sbtc-out)) (try! (swap-sbtc-to-token sbtc-out))))))
          (asserts! (>= amt-out min-amt-out) ERR-SLIPPAGE)
          (asserts! (> amt-out amt-in) ERR-NO-PROFIT)
          (try! (as-contract? ((with-ft 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token "welshcorgicoin" amt-out))
            (try! (contract-call? 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token transfer amt-out current-contract DEPLOYER none))
...
(define-public (arb-vel-vel-fak
  ...
  (begin
    ...
    (let ((stx-out (try! (as-contract? ((with-ft 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token "welshcorgicoin" amt-in)) (try! (swap-token-to-stx-velar amt-in)))))
          (sbtc-out (try! (as-contract? ((with-stx stx-out)) (try! (swap-stx-to-sbtc-velar stx-out)))))
          (amt-out (try! (as-contract? ((with-ft 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token "sbtc-token" sbtc-out)) (try! (swap-sbtc-to-token sbtc-out))))))
          (asserts! (>= amt-out min-amt-out) ERR-SLIPPAGE)
          (asserts! (> amt-out amt-in) ERR-NO-PROFIT)
          (try! (as-contract? ((with-ft 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token "welshcorgicoin" amt-out))
...
;; Bitflow token-STX helpers (WELSH has a Bitflow WELSH-STX pool)
...
;; Bitflow simulate helpers (WELSH has a Bitflow WELSH-STX pool)
...
;; ALEX swap-helper-a routes ALEX / wstx / wcorgi through the caller inside one call
...
(define-public (arb-fak-bit-alex
  ...
  (begin
    ...
    (let ((sbtc-out (try! (as-contract? ((with-ft 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token "welshcorgicoin" amt-in)) (try! (swap-token-to-sbtc amt-in)))))
          (stx-out (try! (as-contract? ((with-ft 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token "sbtc-token" sbtc-out)) (try! (swap-sbtc-to-stx sbtc-out)))))
          (amt-out (try! (as-contract? ((with-stx stx-out) (with-ft 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-alex "alex" ALEX-PASSTHROUGH) (with-ft 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-wstx-v2 "wstx" ALEX-PASSTHROUGH) (with-ft 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-wcorgi "wcorgi" ALEX-PASSTHROUGH)) (try! (swap-stx-to-token-alex stx-out))))))
...
(define-public (arb-alex-bit-fak
  ...
  (begin
    ...
    (let ((stx-out (try! (as-contract? ((with-ft 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token "welshcorgicoin" amt-in) (with-ft 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-alex "alex" ALEX-PASSTHROUGH) (with-ft 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-wstx-v2 "wstx" ALEX-PASSTHROUGH) (with-ft 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-wcorgi "wcorgi" ALEX-PASSTHROUGH)) (try! (swap-token-to-stx-alex amt-in)))))
          (sbtc-out (try! (as-contract? ((with-stx stx-out)) (try! (swap-stx-to-sbtc stx-out)))))
          (amt-out (try! (as-contract? ((with-ft 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token "sbtc-token" sbtc-out)) (try! (swap-sbtc-to-token sbtc-out))))))
...
;; as-contract? returns the body wrapped in a response, so these return (ok true).
...
(define-public (rescue-sbtc (amount uint))
  (begin
    ...
    (as-contract? ((with-ft 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token "sbtc-token" amount))
      (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token transfer amount current-contract DEPLOYER none))
    )
  )
)
...
(define-public (rescue-token (amount uint))
  (begin
    (asserts! (is-eq tx-sender DEPLOYER) ERR-NOT-AUTHORIZED)
    (as-contract? ((with-ft 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token "welshcorgicoin" amount))
      (try! (contract-call? 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token transfer amount current-contract DEPLOYER none))
    )
  )
)

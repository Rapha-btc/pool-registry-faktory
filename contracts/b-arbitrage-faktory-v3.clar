...
(define-public (arb-fak-bit-alex
  ...
  (begin
    ...
    (let ((sbtc-out (try! (as-contract? ((with-ft B B-ASSET token-in))
      ...
      (ok { token-in: token-in, token-out: token-out }))))
...
(define-public (arb-alex-bit-fak
  ...
  (begin
    (try! (contract-call? B transfer token-in tx-sender current-contract none))
    (let ((stx-out (try! (as-contract? ((with-ft B B-ASSET token-in))
...
(define-private (swap-stx-to-token (stx-amount uint))
  (let ((result (try! (contract-call? ALEX-POOL swap-x-for-y WSTX-V2 WBFAKTORY u100000000 stx-amount none))))
    (ok (get dy result))))

(define-private (swap-token-to-stx (token-amount uint))
  (let ((result (try! (contract-call? ALEX-POOL swap-y-for-x WSTX-V2 WBFAKTORY u100000000 token-amount none))))
    (ok (get dx result))))
...
(define-read-only (simulate-sbtc-to-stx (sbtc-amount uint))
  (let ((pool (unwrap-panic (contract-call?
        'SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.xyk-pool-sbtc-stx-v-1-1 get-pool)))
      (x-balance (get x-balance pool)) (y-balance (get y-balance pool))
      (protocol-fee (get x-protocol-fee pool)) (provider-fee (get x-provider-fee pool)) (BPS u10000)
...
(define-read-only (simulate-stx-to-token (stx-amount uint))
  (let ((fee (/ (+ (* stx-amount u500000) u99999999) u100000000))
...
(define-read-only (simulate-token-to-stx (token-amount uint))
  (let ((fee (/ (+ (* token-amount u500000) u99999999) u100000000))
    ...
    (unwrap-panic (contract-call?
      ...
      'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-wstx-v2
      'SP1KK89R86W73SJE6RQNQPRDM471008S9JY4FQA62.token-wbfaktory
      u100000000 token-net))))

(define-read-only (simulate-stx-to-sbtc (stx-amount uint))
  (let ((pool (unwrap-panic (contract-call?
        'SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.xyk-pool-sbtc-stx-v-1-1 get-pool)))
      (x-balance (get x-balance pool)) (y-balance (get y-balance pool))
      (protocol-fee (get y-protocol-fee pool)) (provider-fee (get y-provider-fee pool)) (BPS u10000)
...
(define-read-only (simulate-sbtc-to-stx-velar (sbtc-amount uint))
  (let ((pool (unwrap-panic (contract-call?
          'SP20X3DC5R091J8B6YPQT638J8NR1W83KN6TN5BJY.univ2-pool-v1_0_0-0070 get-pool)))
        (r0 (get reserve0 pool)) (r1 (get reserve1 pool))
...
(define-read-only (check-fak-bit-alex (token-in uint))
  (let ((sbtc-estimate (simulate-token-to-sbtc token-in))
  ...
  (ok { token-in: token-in, sbtc-out: sbtc-estimate, stx-out: stx-estimate,
        token-out: token-estimate, profit: profit, profitable: (> token-estimate token-in) })))
...
(define-read-only (check-alex-bit-fak (token-in uint))
  (let ((stx-estimate (/ (simulate-token-to-stx token-in) u100))
        (sbtc-estimate (simulate-stx-to-sbtc stx-estimate))
        (token-estimate (simulate-sbtc-to-token sbtc-estimate))
        (profit (if (> token-estimate token-in) (- token-estimate token-in) u0)))
  (ok { token-in: token-in, stx-out: stx-estimate, sbtc-out: sbtc-estimate,
        token-out: token-estimate, profit: profit, profitable: (> token-estimate token-in) })))
...
(define-public (rescue-b (amount uint))
  (begin (asserts! (is-eq tx-sender DEPLOYER) ERR-NOT-AUTHORIZED)

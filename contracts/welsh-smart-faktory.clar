;; SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.welsh-smart-faktory (DRAFT - not deployed)
;;
;; Smart split router for WELSH (welshcorgicoin-token, 6 dec), modeled on the
;; deployed pepe-smart-faktory (Clarity 5, as-contract? allowances). Same public
;; interface as b-smart / mia-smart / pepe-smart / leo-smart / lwb-smart.
;; Venue map:
;;   - "fak" leg:  welshcorgicoin-faktory-pool-v2 via fakfun-core-v2 execute
;;     (gated pool, core-v2 approved; LP_REBATE 0.3% inside the quote, sell
;;     fee field is 0 so dy is paid in full - no haircut). The retired
;;     welshcorgicoin-faktory-pool (v1) is NOT used.
;;   - STX-side DEX leg, `flag` picks a whole family:
;;       true  = BitFlow: xyk-pool-sbtc-stx (bridge) + xyk-pool-welsh-stx-v-1-1
;;               (x = WELSH, y = STX, so buying WELSH with STX is swap-y-for-x)
;;       false = Velar univ2-pool-0070 (bridge) + ALEX 2-hop STX <-> ALEX <-> WELSH
;;               through amm-pool-v2-01 swap-helper-a (wstx-v2 / alex / wcorgi,
;;               factors 1e8; 8-dec: *100 in, /100 out, as leo-smart)
;; Not wired (dust): Velar STX-WELSH old-core pool (~2.7k STX), Velar WELSH-aeUSDC.
;; The dex split ratio weighs the fak pool's sBTC side against the DEX leg's
;; STX depth (BitFlow: pool y-balance; ALEX: the wcorgi hop's ALEX balance
;; converted to STX at the STX/ALEX spot). Safety is always the signed
;; min-out, never the ratio.
...
;; define-fungible-token names, NOT contract names.
(define-constant TOKEN-ASSET "welshcorgicoin")
(define-constant SBTC-ASSET "sbtc-token")

(define-constant CORE 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.fakfun-core-v2)
(define-constant POOL 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.welshcorgicoin-faktory-pool-v2)
...
;; x = WELSH, y = STX
(define-constant XYK-TOKEN-POOL 'SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.xyk-pool-welsh-stx-v-1-1)

(define-constant ALEX-AMM 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01)
(define-constant ALEX-WSTX 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-wstx-v2)
(define-constant ALEX-TOKEN 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-alex)
(define-constant ALEX-WCORGI 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-wcorgi)
(define-constant ALEX-FACTOR u100000000)
;; ALEX's 2-hop helper routes the intermediate ALEX plus the wstx / wcorgi
;; wrapper tokens THROUGH the caller inside one call, so those pass-through
;; assets need allowances too. They never rest in this contract between calls,
;; so the cap only bounds what ALEX's own code can move within that call. The
;; allowances are declared on every DEX-leg call regardless of flag; on the
;; BitFlow path nothing draws on them.
(define-constant ALEX-PASSTHROUGH u340282366920938463463374607431768211455)
    ...
    (let (
      ...
      (let (
          ...
          (token-from-alex (if (> stx-from-dex u0)
            (try! (as-contract?
              ((with-stx stx-from-dex) (with-ft ALEX-TOKEN "alex" ALEX-PASSTHROUGH) (with-ft ALEX-WSTX "wstx" ALEX-PASSTHROUGH) (with-ft ALEX-WCORGI "wcorgi" ALEX-PASSTHROUGH))
    ...
    (let (
      ...
      (let (
          ...
          (token-from-alex (if (> alex-amount u0)
            (try! (as-contract?
              ((with-stx alex-amount) (with-ft ALEX-TOKEN "alex" ALEX-PASSTHROUGH) (with-ft ALEX-WSTX "wstx" ALEX-PASSTHROUGH) (with-ft ALEX-WCORGI "wcorgi" ALEX-PASSTHROUGH))
    ...
    (let (
      ...
      (let (
          ...
          (stx-from-alex (if (> alex-amount u0)
            (try! (as-contract?
              ((with-ft TOKEN TOKEN-ASSET alex-amount) (with-ft ALEX-TOKEN "alex" ALEX-PASSTHROUGH) (with-ft ALEX-WSTX "wstx" ALEX-PASSTHROUGH) (with-ft ALEX-WCORGI "wcorgi" ALEX-PASSTHROUGH))
...
(define-read-only (get-fak-sbtc-token-liquidity)
  (let ((pool-data (contract-call?
      'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.welshcorgicoin-faktory-pool-v2
      get-reserves-quote
    )))
    (get dx pool-data)
  )
...
;; STX depth of the DEX leg, native uSTX. BitFlow: welsh-stx y-balance (STX = y).
;; ALEX: the alex/wcorgi hop's ALEX balance converted to STX at the STX/ALEX
;; spot (the shared STX/ALEX hop is far deeper, so the wcorgi hop bounds the route).
(define-read-only (get-dex-stx-token-liquidity (flag bool))
  (if flag
    ...
    (let (
        (wcorgi-pool (unwrap-panic (contract-call?
          'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01 get-pool-details
          'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-alex
          'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-wcorgi u100000000
        )))
        (stx-alex-pool (unwrap-panic (contract-call?
          'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01 get-pool-details
          'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-wstx-v2
          'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-alex u100000000
        )))
        (alex-depth (get balance-x wcorgi-pool))
        (stx-balance (get balance-x stx-alex-pool))
        (alex-balance (get balance-y stx-alex-pool))
      )
      (if (> alex-balance u0)
        ;; 8-dec STX -> ustx
        (/ (* alex-depth stx-balance) alex-balance u100)
        u0
      )
    )
  )
...
(define-private (swap-token-to-sbtc (token-amount uint))
  (let (
      (result (try! (contract-call? CORE execute POOL token-amount (some 0x01))))
...
(define-private (swap-sbtc-to-token (sbtc-amount uint))
  (let ((result (try! (contract-call? CORE execute POOL sbtc-amount (some 0x00)))))
    (ok (get dy result))
  )
)
...
;; STX -> WELSH on the STX-side DEX. BitFlow xyk-pool-welsh-stx: WELSH = X,
;; STX = Y, so sending STX is swap-y-for-x. ALEX: wstx -> alex -> wcorgi, 8-dec.
(define-private (swap-stx-to-token
    (stx-amount uint)
    (flag bool)
  )
  (if flag
    (let ((dx (try! (contract-call? XYK-CORE swap-y-for-x XYK-TOKEN-POOL TOKEN STX-TOKEN
        stx-amount u1
      ))))
      (ok dx)
    )
    (let ((token-8dec (try! (contract-call? ALEX-AMM swap-helper-a ALEX-WSTX ALEX-TOKEN
        ALEX-WCORGI ALEX-FACTOR ALEX-FACTOR (* stx-amount u100) none
      ))))
      (ok (/ token-8dec u100))
    )
  )
...
;; WELSH -> STX. BitFlow: swap-x-for-y (send X = WELSH, get Y = STX). ALEX:
;; wcorgi -> alex -> wstx, 8-dec.
(define-private (swap-token-to-stx
    (token-amount uint)
    (flag bool)
  )
  (if flag
    (let ((dy (try! (contract-call? XYK-CORE swap-x-for-y XYK-TOKEN-POOL TOKEN STX-TOKEN
        token-amount u1
      ))))
      (ok dy)
    )
    (let ((stx-8dec (try! (contract-call? ALEX-AMM swap-helper-a ALEX-WCORGI ALEX-TOKEN
        ALEX-WSTX ALEX-FACTOR ALEX-FACTOR (* token-amount u100) none
      ))))
      (ok (/ stx-8dec u100))
    )
  )
...
;; net received = dy - output-side fee (u0 on this pool template, kept for parity)
(define-read-only (simulate-token-to-sbtc (token-amount uint))
  (let ((q (contract-call?
      'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.welshcorgicoin-faktory-pool-v2
      get-swap-quote token-amount (some 0x01)
    )))
    (- (get dy q) (get fee q))
  )
)
...
(define-read-only (simulate-sbtc-to-token (sbtc-amount uint))
  (get dy
    (unwrap-panic (contract-call?
      'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.welshcorgicoin-faktory-pool-v2 quote
...
;; STX -> WELSH quote. BitFlow: y-side fees, then x out. ALEX: get-helper-a,
;; 8-dec (zero-guarded: the helper rejects a zero input).
    ...
    (if (> stx-amount u0)
      (/
        (unwrap-panic (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01
          get-helper-a 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-wstx-v2
          'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-alex
          'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-wcorgi u100000000
          u100000000 (* stx-amount u100)
        ))
        u100
      )
      u0
...
;; WELSH -> STX quote, same two venues the other way round.
(define-read-only (simulate-token-to-stx
  ...
  (if flag
    ...
    (if (> token-amount u0)
      (/
        (unwrap-panic (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01
          get-helper-a 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-wcorgi
          'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-alex
          'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-wstx-v2 u100000000
          u100000000 (* token-amount u100)
        ))
        u100
      )
      u0
...
;; "BitFlow" = flag true (BitFlow bridge + BitFlow welsh-stx); "Velar" = flag
;; false (Velar bridge + ALEX 2-hop).

(use-trait faktory-account .faktory-account-traits.account-configuration)

(define-constant ATTESTOR_DEPLOYER 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM) ;; 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22)
...
(define-map agent-to-account principal principal)
...
(define-public (auto-register-account (owner principal) (agent principal))
  (begin  
...
(define-public (register-account (account <faktory-account>))
  (let (
    (account-address (contract-of account))
    (config (unwrap! (contract-call? account get-configuration) ERR_GET_CONFIG_FAILED))
    (owner (get owner config))
    (agent (get agent config))
  )
    (asserts! (is-eq tx-sender ATTESTOR_DEPLOYER) ERR_NOT_AUTHORIZED_DEPLOYER)
    (try! (validate-principals account-address owner))
    (do-register-account account-address owner agent)
  )
)

(define-private (do-register-account (account principal) (owner principal) (agent principal))
  (begin
    (asserts! (map-insert account-registry account {
      owner: owner,
      agent: agent,
      attestation-level: u1}) ERR_ALREADY_REGISTERED)
    (asserts! (map-insert owner-to-account owner account) ERR_ALREADY_REGISTERED)
    (asserts! (map-insert agent-to-account agent account) ERR_ALREADY_REGISTERED)
    (print {
      notification: "account-registered",
      payload: {
        account: account,
        owner: owner,
        agent: agent,
...
(define-public (attest-account (account principal))
  (let ((registry-entry (unwrap! (map-get? account-registry account) ERR_ACCOUNT_NOT_FOUND))
    ...
    (asserts! (map-insert account-attestations { account: account, attestor: tx-sender } true) ERR_ALREADY_REGISTERED)
    (map-set account-registry account (merge registry-entry { attestation-level: new-level }))   
      (print {

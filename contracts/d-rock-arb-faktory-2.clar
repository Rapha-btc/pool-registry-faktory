...
(define-private (swap-token-to-sbtc (amount uint))
  (let (
      (result (try! (contract-call? .rock-faktory-pool-3 execute amount (some 0x01))))
...
(define-private (swap-sbtc-to-token (sbtc-amount uint))
  (let ((result (try! (contract-call? .rock-faktory-pool-3 execute sbtc-amount (some 0x00)))))
    (ok (get dy result))
  )
)
...
(define-read-only (simulate-token-to-sbtc (amount uint))
  (let ((q (contract-call? .rock-faktory-pool-3 get-swap-quote amount (some 0x01))))
    (- (get dy q) (get fee q))
  )
)

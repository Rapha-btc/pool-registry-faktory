...
// =====================================================================
// Act 1 -- contracts are LIVE (block 8,895,371); just fund the deployer.
// Also exercises the deployed rock-smart-faktory-2 / rock-arb-faktory-2
// after the gate opens.
// =====================================================================
const SMART_CID = `${DEPLOYER}.rock-smart-faktory-2`;
const ARB_CID = `${DEPLOYER}.rock-arb-faktory-2`;
...
// live routers against the open pool-3
call("smart-2 buy-with-sbtc (pure fak leg)", DEP1, SMART_CID, "buy-with-sbtc",
  [uintCV(20_000n), uintCV(1n), uintCV(100n), boolCV(true)], /^\(ok /);
call("smart-2 sell-for-sbtc (pure fak leg)", DEP1, SMART_CID, "sell-for-sbtc",
  [uintCV(1_000_000_000_000n), uintCV(1n), uintCV(100n), boolCV(true)], /^\(ok /);
call("smart-2 buy-with-sbtc (pure velar leg, bitflow bridge)", DEP1, SMART_CID, "buy-with-sbtc",
  [uintCV(20_000n), uintCV(1n), uintCV(0n), boolCV(true)], /^\(ok /);
evalc("arb-2 check-fak-bit-vel(1M ROCK)",
  `(contract-call? '${ARB_CID} check-fak-bit-vel u1000000000000)`, "arbCheck");
call("arb-2 execute (profit-or-revert accepted)", HIGHROLLER, ARB_CID, "arb-fak-bit-vel",
  [uintCV(1_000_000_000_000n), uintCV(0n)],
  (s) => s.startsWith("(ok") || /err u10\d\d/.test(s), "arbExec");

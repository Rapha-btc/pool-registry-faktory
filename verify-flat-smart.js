// verify-flat-smart.js
// Self-verifying stxer mainnet-fork harness for flatearth-smart-faktory (DRAFT).
...
const C = `${DEPLOYER}.flatearth-smart-faktory`;
...
const MIA_HOLDER = "SP3W69VDG9VTZNG7NTW1QNCC1W45SNY98W1JSZBJH"; // 20k FLAT (6dec); deployer EOA holds ~31.5M

const SBTC_AMOUNT = 100000n; // 100k sats
const STX_AMOUNT = 100000000n; // 100 STX (6dec)
const MIA_AMOUNT = 20000000000n; // 20k FLAT (6dec); deployer EOA holds ~31.5M
...
steps.push({ label: "deploy flatearth-smart-faktory (Clarity 5)", kind: "deploy" });
...
for (const [label, code] of [
  ...
  ["sim stx->token (velar 0003)", `(simulate-stx-to-token u${STX_AMOUNT} true)`],
  ["sim token->stx (velar 0003)", `(simulate-token-to-stx u${MIA_AMOUNT} true)`],
